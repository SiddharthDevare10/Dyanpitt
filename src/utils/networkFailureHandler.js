...
class NetworkFailureHandler {
  ...
  async handleRequest(requestFn, options = {}) {
    ...
    try {
      ...
      if (this.isNetworkError(error)) {
        console.log(`📡 Network error detected for request ${id}`);
  ...
  async executeWithRetry(requestFn, options = {}) {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (attempt > 0) {
          console.log(`✅ Request ${id} succeeded on attempt ${attempt + 1}`);
        }
        ...
        if (attempt > maxRetries || !this.isRetryableError(error)) {
          console.error(`❌ Request ${id} failed after ${attempt} attempts:`, error);
          throw error;
        }
        
        console.log(`⏳ Request ${id} failed, retrying in ${currentDelay}ms (attempt ${attempt}/${maxRetries})`);
  ...
  queueRequest(requestFn, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(`📥 Queued request ${id} (priority: ${priority}, queue size: ${this.retryQueue.length})`);
    });
  }
  ...
  async retryFailedRequests() {
    ...
    console.log(`🔄 Manually retrying ${failedRequestsArray.length} failed requests`);
