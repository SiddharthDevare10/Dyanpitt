...
class TourLinkingService {
  ...
  static async linkTourRequestsToUser(email, userId, dyanpittId = null) {
    try {
      ...
      console.log(`✅ Linked ${updateResult.modifiedCount} tour requests to user ${email}`);
      ...
      console.error('Error linking tour requests:', error);
      return {
        success: false,
        linkedCount: 0,
        error: error.message,
  ...
  static async updateTourRequestsWithDyanpittId(email, dyanpittId) {
    try {
      ...
      console.log(`✅ Updated ${updateResult.modifiedCount} tour requests with Dyanpitt ID ${dyanpittId}`);
  ...
  static async checkUserTours(email) {
    try {
      ...
      console.error('Error checking user tours:', error);
      return {
        success: false,
        hasTours: false,
        tourCount: 0,
