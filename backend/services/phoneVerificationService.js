...
class PhoneVerificationService {
  ...
  async sendOTP(phoneNumber) {
    try {
      ...
      console.log(`📱 OTP sent to ${normalizedPhone}: ${otp} (Development only)`);
  ...
  verifyOTP(phoneNumber, otp) {
    try {
      ...
      if (new Date() > storedData.expiresAt) {
        console.log(`❌ OTP expired for phone: ${normalizedPhone}`);
        this.otps.delete(normalizedPhone);
        return false;
      }
      ...
      if (storedData.attempts >= 5) {
        console.log(`❌ Too many attempts for phone: ${normalizedPhone}`);
        this.otps.delete(normalizedPhone);
        return false;
      }
      ...
      if (storedData.otp !== otp.toString()) {
        console.log(`❌ Invalid OTP for phone: ${normalizedPhone}`);
        return false;
      }
  ...
  clearOTP(phoneNumber) {
    const normalizedPhone = this.normalizePhoneNumber(phoneNumber);
    this.otps.delete(normalizedPhone);
    console.log(`🧹 Cleared OTP for phone: ${normalizedPhone}`);
  }
  ...
  async sendSMS(phoneNumber, otp) {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`📱 MOCK SMS to ${phoneNumber}: Your Dyanpitt verification code is: ${otp}. Valid for 10 minutes.`);
      ...
      */
      ...
      console.log(`📱 SMS would be sent to ${phoneNumber}: ${otp}`);
      return {
        success: true,
  ...
  cleanupExpiredOTPs() {
    ...
    if (cleaned > 0) {
      console.log(`🧹 Cleaned up ${cleaned} expired phone OTPs`);
    }
